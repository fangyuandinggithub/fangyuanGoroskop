...
- (void)setScoresArray:(NSArray *)scoresArray
{
    ...
    [self.subviews enumerateObjectsUsingBlock:^(__kindof UIView * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        if([obj isKindOfClass:[RADotView class]]){
...
- (NSArray *)convertLengthsFromScore:(NSArray *)scoreArray
{
    ...
    if(lengthArray.count >5){
      return   [lengthArray subarrayWithRange:NSMakeRange(0, 5)];
    }
    return lengthArray ;
...
- (void)drawBgPentagon
{
    ...
    NSArray *titleArray = [NSArray arrayWithObjects:@"love",@"family",@"health",@"wealth",@"total", nil];
    ...
    for (int i = 0; i < [titleArray count]; i++) {
        ...
        label.bounds = CGRectMake(0, 0, 50, 11);
