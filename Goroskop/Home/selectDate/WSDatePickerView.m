...
- (IBAction)canBtnClick:(id)sender {
...
-(void)setupUI {
    ...
    self.datePickerColor = [UIColor whiteColor];
...
-(void)addLabelWithName:(NSArray *)nameArr {
    ...
    if (!_dateLabelColor) {
        _dateLabelColor =  [UIColor whiteColor];
    }
    
    for (int i=0; i<nameArr.count; i++) {
        CGFloat labelX = kPickerSize.width/(nameArr.count*2)+18+kPickerSize.width/nameArr.count*i;
        UILabel *label = [[UILabel alloc]initWithFrame:CGRectMake(labelX, self.showYearView.frame.size.height/2-15/2.0, 50, 15)];
...
- (NSInteger)numberOfComponentsInPickerView:(UIPickerView *)pickerView
{
    switch (self.datePickerStyle) {
        ...
        case DateStyleShowYearMonth:
            ...
            return 2;
        case DateStyleShowMonthDay:
            [self addLabelWithName:@[@"Month",@"Date"]];
            return 2;
        case DateStyleShowHourMinute:
...
-(UIView *)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(UIView *)view {
    ...
    if (!_datePickerColor) {
        _datePickerColor = [UIColor whiteColor];
    }
...
-(void)setDoneButtonColor:(UIColor *)doneButtonColor {
    ...
    self.cancalBtn.backgroundColor = doneButtonColor;
